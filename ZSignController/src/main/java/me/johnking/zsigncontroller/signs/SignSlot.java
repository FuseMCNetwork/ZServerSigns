...
import me.johnking.zsigncontroller.packet.StartServerPacket;
    ...
    public void setServer(SignServer server) {
        ...
        if(this.server != null) {
            ...
            if(server == null) {
                ...
                if(!this.vip) {
                    ZNetworkPlugin.getInstance().sendEvent("serversigns_startserver", new StartServerPacket(this.server.getServer().getServerID()));
                }
            }
        }
        if(server != null) {
